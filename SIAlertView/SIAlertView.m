...
- (void)validateLayout
{
    ...
    if (self.winBgImageView){
        self.winBgImageView.frame = CGRectMake(0, 0, self.containerView.frame.size.width, self.containerView.frame.size.height);
    }
...
- (void)setupContainerView
{
    ...
    if (!self.winBgImageView){
        self.winBgImageView = [[UIImageView alloc]initWithImage:[UIImage imageNamed:@"alert_bg.png"]];
        self.winBgImageView.frame = self.bounds;
        [self.containerView addSubview:self.winBgImageView];
    }
}
...
- (UIButton *)buttonForItemIndex:(NSUInteger)index
{
	...
	RORNormalButton *button = [RORNormalButton buttonWithType:UIButtonTypeCustom];
	button.tag = index;
...
//        return;
//    }
    _viewBackgroundColor = [UIColor clearColor];//viewBackgroundColor;
    self.containerView.backgroundColor = [UIColor clearColor];//viewBackgroundColor;
